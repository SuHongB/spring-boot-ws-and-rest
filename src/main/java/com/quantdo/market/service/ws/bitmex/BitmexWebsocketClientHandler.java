 ...
 * @author suhongbin
 */
public class BitmexWebsocketClientHandler extends AbstractWebsocketClientHandler {
	...
	public BitmexWebsocketClientHandler(AbstractWebsocketClient abstractWebsocketClient,HashOperations<String,String,String> redis,String cacheType) {
	...
	protected void onReceive(Channel ch, String text) {
		if(text.contains(BitstampMessageEnum.ERROR.getValue())){
			logger.error("Bitmex交易所收到订阅失败消息:{}",text);
		}else if(text.contains(BitstampMessageEnum.SUCCEES.getValue())){
			logger.info("Bitmex交易所收到订阅成功消息:{}",text);
		}else if(text.contains(BitstampMessageEnum.PING.getValue())){
			logger.info("Bitmex交易所收到心跳消息:{}",BitstampMessageEnum.PONG.getValue());
		}else{
			logger.debug("Bitmex交易所收到订阅消息:{}",text);
